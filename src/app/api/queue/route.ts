...
import { Appointment as AppointmentType, AppointmentStatus, QueueState } from "@/types";
...
export async function GET() {
  try {
    ...
    let queueDoc = await QueueModel.findOne({ queueId: "main_opd_queue" }).lean();
    if (!queueDoc) {
    ...
    const activeList: AppointmentType[] = appointments.length > 0
      ? appointments.map((a) => ({
          id: a.appointmentId,
...
export async function POST(req: Request) {
  try {
    ...
    const { action, tokenNumber, inChamber } = body;
    ...
    try {
      ...
      if (action === "next" || action === "skip" || action === "recall") {
        await QueueModel.findOneAndUpdate(
          { queueId: "main_opd_queue" },
          { currentlyServingToken: updatedQueue.currentlyServingToken, lastUpdated: new Date() },
          { upsert: true }
        );
      } else if (action === "setDoctorStatus" && typeof inChamber === "boolean") {
        await QueueModel.findOneAndUpdate(
